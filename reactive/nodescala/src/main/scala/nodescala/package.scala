...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
     ...
     */
    def userInput(message: String): Future[String] = Future { blocking { readLine(message) } }

    /** Creates a cancellable context for an execution and runs it.
     */
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      val s = CancellationTokenSource()
      async { f(s.cancellationToken) }
      s
    }
  }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = f.value flatMap { _.toOption } getOrElse { throw new NoSuchElementException }
